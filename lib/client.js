...
IMAPClient.prototype.connect = function(){
    ...
    if(this.options.secureConnection){
        ...
        if(this.options.socks){
            console.log('using socks connection');

            var SocksClient = require('socks-client');

            var socksOptions = {
                proxy: this.options.socks,
                target: {
                    host: this.host,
                    port: this.port
                },
            ...
            SocksClient.createConnection(socksOptions, function(err, socket, info){
                if(err){
                    throw err;
                }

                console.log('socks connection result: ' + socket);

                client._connection = socket;
                //client._connection.on("connect", client._onConnect.bind(client));
                client._connection.on("error", client._onError.bind(client));
                client._greetingTimeout = setTimeout(client._handleGreetingTimeout.bind(client), client.GREETING_TIMEOUT);
